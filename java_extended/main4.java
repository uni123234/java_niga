...
// Конструктор — це спеціальний метод, який використовується для ініціалізації об'єктів. Він викликається кожен раз, коли створюється об'єкт класу. Конструктори також можуть приймати параметри, які використовуються для ініціалізації полів.

// Зверни увагу, що назва конструктора повинна збігатися з назвою класу. Також конструктор не має повертаючого типу:
...
// Усі класи мають конструктор без параметрів за замовчуваннями — якщо ти не створив(-ла) конструктор власноруч, Java створить його за тебе:
...
//         User user = new User("Bob"); // ми не можемо передати ім'я користувача, адже в нас немає потрібного конструктора з параметрами  
//     }
// }

// Перевантаження конструкторів — це особливість, яка дозволяє класу мати декілька конструкторів із різними параметрами. Це означає, що можна створювати об’єкти одного класу, використовуючи різні набори параметрів, залежно від конкретних потреб.

// Якщо клас має декілька конструкторів із різними параметрами, компілятор використовуватиме відповідний конструктор на основі аргументів, які передаються під час створення об’єкта.
